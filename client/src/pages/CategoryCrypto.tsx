...
import { TrendingUp, TrendingDown, Search, Bitcoin, ArrowLeft } from "lucide-react";
import { Link } from "wouter";

export default function CategoryCrypto() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <div className="border-b border-border bg-gradient-to-r from-orange-500/10 to-orange-600/10 backdrop-blur-sm sticky top-0 z-10">
        <div className="container py-8">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-4">
              <div className="p-4 rounded-lg bg-orange-500/20 border-2 border-orange-500/50">
                <Bitcoin className="h-12 w-12 text-orange-500" />
              </div>
              <div>
                <h1 className="text-4xl md:text-5xl font-bold neon-glow-pink">加密貨幣市場</h1>
                <p className="text-muted-foreground mt-2 text-lg">
                  追蹤比特幣、以太坊及其他加密貨幣的預測市場走勢
                </p>
              </div>
            </div>
      ...
      <div className="container py-8">
        {/* Search */}
        <Card className="p-6 mb-8 bg-card/50 backdrop-blur-sm border-orange-500/20">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input
              placeholder="搜尋加密貨幣市場..."
              value={searchQuery}
        ...
        {isLoading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredMarkets.map((market) => (
              <Link key={market.id} href={`/market/${market.id}`}>
                <Card className="p-6 bg-card/50 backdrop-blur-sm border-border hover:border-orange-500/50 transition-all duration-300 cursor-pointer group h-full">
                  {/* Title */}
          ...
          <Card className="p-12 text-center bg-card/50 backdrop-blur-sm border-border">
            <Bitcoin className="h-16 w-16 mx-auto mb-4 text-muted-foreground opacity-50" />
            <p className="text-lg text-muted-foreground mb-2">
              {searchQuery ? "未找到符合條件的加密貨幣市場" : "暫無加密貨幣市場數據"}
            </p>
            {searchQuery && (
              <Button
                variant="outline"
